class Dashing.ProgressBars extends Dashing.Widget
  ...
  # /
  animateProgressBarContent: (element, from, to,realValue, baseDuration) ->
      ...
      interval = setInterval(
        ->
          ...
          if Math.abs(currentValue - from) >= Math.abs(endpointDifference)
            setProgressBarValue(element, to,realValue)
            clearInterval(interval)
          else
            setProgressBarValue(element, currentValue,realValue)
        stepInterval)
  ...
  # /
  setProgressBarValue = (element, value,realValue) ->
    if (value > 100) 
      value = 100
    else if (value < 0) 
      value = 0
    element.textContent = "￥" + Math.floor(realValue)
